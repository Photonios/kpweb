...
func entryHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		w.WriteHeader(http.StatusUnauthorized)
		writeErrorResponse(w, "no session cookie")
		log.Printf("unauthorized attempt at editing entry, no session cookie")
		return
	}
	...
	if err != nil {
		w.WriteHeader(http.StatusForbidden)
		writeErrorResponse(w, "no valid session")
		log.Printf("unauthorized attempt at editing entry, %s", err)
		return
	}
	...
	if entry == nil {
		...
		log.Printf("entry update for unknown entry %s", entryID)
		return
	}
	...
	if err := json.NewDecoder(r.Body).Decode(&entryData); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		writeErrorResponse(w, "could not decode request body")
		log.Printf("entry update failed: %s", err)
		return
	}

	if err := entry.UpdateData(entryData); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		writeErrorResponse(w, "could not update entry")
		log.Printf("entry update failed: %s", err)
		return
	}

	log.Printf("entry %s updated by session %s", entry.ID, session.ID)
